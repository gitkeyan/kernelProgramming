...
{
	spin_lock(&pidlist_lock);//lock the pid lists before removing taht pid from all lists
	del_pid(current->pid);
	spin_unlock(&pidlist_lock);
	orig_exit_group(status);//call the original exit_group
}
//----------------------------------------------------------------
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	spin_lock(&pidlist_lock);//lock the pid lists because of manipulating mytable
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if(syscall < 0 || syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL){//syscall needs to be valid
		return -EINVAL;
	}
	else if(cmd == REQUEST_SYSCALL_INTERCEPT){
		if(current_uid() == 0){//only if caller is the root
			spin_lock(&pidlist_lock);

			if(table[syscall].intercepted == 1){//if it's already intercepted
				spin_unlock(&pidlist_lock);//prevent deadlock
				return -EBUSY;
			}
			...
			table[syscall].f = sys_call_table[syscall];//store the intercepted function

			set_addr_rw((unsigned long) sys_call_table);
			sys_call_table[syscall] = &interceptor;//replace it with our interceptor
		...
		if(current_uid() == 0){
			...
			spin_lock(&calltable_lock);//lock when manipulating the sys_call_table

			set_addr_rw((unsigned long) sys_call_table);
			sys_call_table[syscall] = table[syscall].f;//restore the intercepted function back to the sys_call_table
			set_addr_ro((unsigned long) sys_call_table);//set back to read only when we are done
		...
		if(pid == 0){//for all all pids
			if(current_uid() == 0){//only the root has the permission
				spin_lock(&pidlist_lock);
			...
			if(pid_task(find_vpid(pid), PIDTYPE_PID) != NULL){//if it's a valid process
				if(current_uid() == 0 || check_pid_from_list(current->pid, pid) == 0){//use current->pid rather than current_uid(), kernel oops otherwise
					spin_lock(&pidlist_lock);
					if(check_pid_monitored(syscall, pid)){
						...
						if(add_pid_sysc(pid, syscall) == -ENOMEM){//ad pid fail bacause of lack of memory
							spin_unlock(&pidlist_lock);
							return -ENOMEM;
						}
						if(table[syscall].monitored == 0){//check if it's monitored already
		...
		if(pid == 0){
			if(current_uid() == 0){
				spin_lock(&pidlist_lock);
				if (table[syscall].monitored == 0) {//when trying to stop monitoring a process that is not monitored
					spin_unlock(&pidlist_lock);
					return -EINVAL;
				}
				else{
			...
			if(pid_task(find_vpid(pid), PIDTYPE_PID) != NULL){
				if(current_uid() == 0 || check_pid_from_list(current->pid, pid) == 0){
					...
					if(check_pid_monitored(syscall, pid) != 0){
						del_pid_sysc(pid, syscall);//stop monitoring this pid for the syscall
					}
					else{
						spin_unlock(&pidlist_lock);
						return -EINVAL;
...
static int init_function(void) {
	...
	spin_lock(&pidlist_lock);
	for(i = 0; i < NR_syscalls; i++){//initialization for bookkeeping data structures
	...
	set_addr_rw((unsigned long) sys_call_table);//set sys_call_table to read write in order to change it
...
static void exit_function(void)
{
	...
	spin_lock(&pidlist_lock);
	for(i = 0; i < NR_syscalls; i++){//stop monitoring all the pids when exit
		destroy_list(i);
	}
	spin_unlock(&pidlist_lock);
